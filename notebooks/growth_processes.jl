...
using Distributions, OffsetArrays, Images, Base64, ColorSchemes, Plots, ImplicitEquations, LaTeXStrings

# ╔═╡ 77381dc0-8f8f-466b-a7f2-a05616df020c
using OrdinaryDiffEq, SpecialFunctions, StaticArrays

# ╔═╡ 1934e6e3-b0be-4c44-accb-259800d7631e
...
function accumulate_corner_growth(W, n=size(W, 1); shifted=true)
...
render(img, format) = HTML("""
	<img src='data:image/gif;base64,$(stringmime(MIME("image/$format"), img))'
		 width=600 height=600 style='image-rendering: pixelated' />
""")
...
color_plot = let n=1000, p=.5, t = 1500
...
# ╔═╡ f9dea8c9-187c-4ad3-b37d-597238db5b8a
let n=1000
	plot(color_plot, ylims=(0, n), xlims=(0, n), yflip=false, size=(700, 700))
end
...
let n=1000, p=.5, t = 2000
	...
	#=plot!(Eq(0) do x, y
		x * ω(y/x, q) - t
	end, xlims=(0, n), ylims=(0, n), fillcolor=:red)
	plot!(lw=2, c=:green, legend=false) do x
		x = x/t
		t * (q - q^2 + 2q * x - q^2 * x -
 			2sqrt(q^2 * x - q^3 * x + q^2 * x^2 - q^3 * x^2))/q^2
	end=#
	plot!(Eq(t) do x, y
		x * ω(y/x, q) + x + y
	end, xlims=(0, n), ylims=(0, n), fillcolor=:green)
...
function pdf_tracy_widom(t; t₀=6., solver=Tsit5(), kw...)
	t ≥ t₀ && return -airy(t)^2
	q₀, q′₀ = airy(t₀), airy(1, t₀)
	u₀ = SA[q₀, q′₀, 0, q₀^2]
	prob = ODEProblem(u₀, (t₀, t)) do (q, q′, I, I′), _, t
		SA[q′, t*q + 2q^3, I′, q^2]
	end
	sol = solve(prob, solver, reltol=1e-14, abstol=1e-14, save_everystep=false, kw...)
	_, _, I, I′ = sol[end]
	return -I′*exp(-I)
end
...
let n=1000, p=.5, t = 2000
	...
	plot()
	global df = DataFrame()
	for t in 100:100:2000
		I = t - 5 .≤ T .≤ t + 5
	    plot!(ARGB.(I, 0., 0., 0.), yflip=false)
		pts = CartesianIndices(T)[I]
		x, y = map(I -> Tuple(I)[1]-1, pts), map(I -> Tuple(I)[2]-1, pts)
		a, b, c = Optim.minimizer(optimize(zeros(3), fill(Inf, 3), [1/n, 1/n, √2*n], Fminbox(LBFGS()); autodiff = :forward) do (a, b, c)
			norm(@. a * (x/√2 + y/√2 - c)^2 + b * (-x/√2 + y/√2)^2 - 1)
		end)
		plot!(Eq(1) do x, y
			a * (x/√2 + y/√2 - c)^2 + b * (-x/√2 + y/√2)^2
		end, xlims=(0, n), ylims=(0, n))
		push!(df, (; a, b, c))
	end
	plot!()
end
...
#=╠═╡
begin
	p1 = plot(100:100:2000, inv.(df.a), label=L"1/a")
	p2 = plot(100:100:2000, inv.(df.b), label=L"1/b")
	p3 = plot(100:100:2000, df.c, label=L"c")
	plot(p1, p2, p3; layout=@layout([A B C]), plot_title=L"Best fit for $a(x\prime-c)^2 + by\prime^2 = 1$", size=(1000, 300))
end
  ╠═╡ =#

# ╔═╡ 4b0651aa-649a-411a-a387-61914013ba6d
md"""
Iterates over all indices $(i, j)$ for which $\lvert i \rvert + \lvert j \rvert = n$
"""

# ╔═╡ 88dc5784-4868-4cd7-8648-d274c8cbd215
function diamond(n, origin=CartesianIndex(0, 0))
	len = n == 0 ? 1 : 4n
	return Iterators.map(0:len-1) do i
		origin + CartesianIndex(
			i < 2n ? n - i : i - 3n,
			i < n ? i : i < 3n ? 2n - i : i - 4n,
		)
	end
end

# ╔═╡ c1aaeb61-d0ff-4fed-bf37-81764c93740d
neighbors(origin) = diamond(1, origin)

# ╔═╡ ea328bb6-6bc4-40ca-b420-6c2df56f70a1
function accumulate_central_growth(W, n; max_backprop=typemax(Int))
	T = fill!(similar(W), typemax(Int))
	T[0, 0] = 0
	for i in 1:2n
		for j in i:-1:max(1, i-max_backprop)#1
			for I in diamond(j)
				I in CartesianIndices(W) || continue
				T[I] = W[I] + minimum(I -> get(T, I, typemax(Int)), neighbors(I)) + 1
			end
		end
	end
	return T
...
# ╔═╡ 875893bd-9d79-4891-abb9-7128d44e07de
p = .1

# ╔═╡ ec0ebf00-b8f7-416a-9ad9-defcf592f095
n = 100

# ╔═╡ ea3047cc-d867-4873-ae4c-0c8fedc93cdd
W = OffsetArray(rand(Geometric(p), (2n+1, 2n+1)), (-n:n, -n:n));

# ╔═╡ 529364f3-1690-40c1-802b-bf327cedd89e
T = accumulate_central_growth(W, n);

# ╔═╡ d55bd863-33e0-4d61-854d-a709e0d1386c
plot_growth(T)

# ╔═╡ 8b1d6d0a-2e2d-4ceb-87c3-948952c964e3
plot_growth(T; color=true)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "ac0b23c06f021557122982c9bbd37eadd11571bd"
...
git-tree-sha1 = "78214be3cb1fb79640ccd40d73bc304958717ee1"
